...
别名调用不需要 DeepSeek 看图：内置图库的文字来自 macOS Vision 本地 OCR，只有能唯一指向一张图片的文本才会自动成为别名；配置为 `LONGTU_QQ_ADMIN_USERS` 的超级管理员可直接覆盖别名。QQ 用户发送“发赛尔号”时，后端直接按手动绑定精确发图；发送“辱骂一下赛尔号”等正常对话时仍由模型生成文字，但附图优先使用“赛尔号”对应图片。普通对话中的附图联动只使用管理员手动别名，不会因为大量 OCR 自动别名而误命中。绑定信息和资源标识只存在于后台，不会回复到群里。
