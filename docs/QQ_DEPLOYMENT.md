...
`LONGTU_QQ_ACTIVE_REPLY_GROUPS` 留空时沿用 Bridge 的 `LONGTU_QQ_ALLOWED_GROUPS` 范围。判定器先输出 `must/may/no`：只有明确点名/引用机器人、紧迫风险和可能造成现实损失的关键纠错属于 `must`，会绕过可选插话限制；普通公开提问/求助、话题延续和有趣话题都属于 `may`。普通 `may` 默认使用 0.30 候选概率，公开问句使用 0.60；同群冷却 120 秒，每群每小时最多 6 次。“能的”“好的”“确实”“哈哈”“收到”和纯表情等低信息附和由程序直接静默，不调用 AI 判定和联网检索；明确点名、引用机器人或真正问句不受该层影响。20 秒内至少 4 条消息且涉及 2 名发送者时只抑制普通趣味插话，不压掉公开问句。机器人发言后连续 3 条消息没人接它时，`may` 暂停 600 秒，超时后可参与新话题，不再永久退场。判定器仍会忽略明确发给其他成员的 `@`/引用、图片、斜杠指令和机器人自身消息。判定模型故障时，只有点名或引用机器人按 `must` 放行，普通公开问句默认沉默，不影响原有明确 `@机器人` 的被动回复。

真人明确 `@` 或引用机器人并收到回复后，会按群号开启默认 100 秒的群级连续话题窗口。原发起者和其他真人都可以承接同一话题，但不会逐条必回：原发起者的明确相关追问优先回复，其他相关发言先由语义判定，再经过默认 18 秒群级节流和 0.60 概率；每段窗口最多主动补充 4 次。只有实际的相关话题回复才会重新计算 100 秒，无关发言、附和与转向他人既不回复也不续期。窗口内再次明确 `@` 会续期，但不会更换原发起者或重置主动补充次数；群内连续 `@` 默认采用 5 秒短节流，防止多人同时点名造成瞬间刷屏。发起者明确说“别再回复了”“停止回复”“结束这个话题”“到此为止”等时由程序立即关闭整段话题，其他参与者提出同类要求时只退出其本人，结束指令不受连续 `@` 节流影响。登记在 `LONGTU_QQ_PEER_BOT_USERS` 中的 Bot 永远不能加入或续期真人话题窗口。`LONGTU_QQ_ADMIN_USERS` 中的超级管理员拥有群级最高优先级：无论机器人正在和谁聊，超管发出明确结束指令都会关闭该群话题窗口、暂停该群普通主动回复 100 秒，并把所有已登记 peer Bot 直接压到硬上限，在 5 分钟循环窗口到期前不再续聊；新的真人明确 `@` 仍可开启一段全新对话。
