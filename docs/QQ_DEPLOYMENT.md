...
首次主动回答也会开启默认 100 秒话题窗口。后续成功生成相关回复才续期并计数；生成失败不消耗续聊轮次。窗口内一般 `may`/`help` 补充仍受 18 秒间隔与最多 4 次限制，非热聊的 `may` 还需 0.60 概率；真人热聊的 `may` 经价值复核后免随机；明确相关 `followup` 使用独立的三轮额度。再次明确 @/引用后恢复三轮续聊额度，保留话题发起者和一般补充计数；连续 @ 保留 5 秒短节流。机器人不会在没人发言时自己连发三轮。
