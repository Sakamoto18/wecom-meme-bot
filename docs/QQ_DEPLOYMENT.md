...
龙图库选中的图片按 QQ 表情类型发送（OneBot `image.sub_type=1`），关键词、tag、图片匹配、随机选图和去重仍使用原有本地图库。Bridge 使用独立组件保留该标记，避免 AstrBot 把它转换回普通图片；视频分享卡、图文预览仍是普通图片。具体显示尺寸由 QQ 客户端决定，原始图片不会被缩小或覆盖。
