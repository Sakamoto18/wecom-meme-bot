...
- 群内只有 `/add`、`/tag`、`/del` 和超管硬终止命令 `/stop` 会进入本项目；其他以 `/` 开头的 AstrBot/插件指令都会被 Bridge 停止。
...
日报还会按每次 LLM 调用发生的北京时间计算 DeepSeek 人民币费用，而不是拿总 Token 乘统一价格。当前官方 `deepseek-v4-flash` 价格（2026-08-25 核对）为：工作日 09:00–12:00、14:00–18:00 的高峰时段，缓存命中输入/缓存未命中输入/输出分别是 0.10/3.0/9.0 元每百万 Token；其余空闲时段分别是 0.05/1.5/4.5 元每百万 Token。日报显示总估算费用、三类 Token 费用、高峰/空闲费用，并按费用对群排行。金额不包含 Exa 联网搜索费用；价格可能变化，需以 [DeepSeek 官方价格页](https://api-docs.deepseek.com/zh-cn/quick_start/pricing/) 为准。

任一超管可以在 QQ 私聊或群聊中发送 `/usage-report`，立即把“今天 00:00 至当前”的累计测试日报私聊推送给所有日报收件人。手动触发只读取快照，不会清零或移动统计起点；同一天再次触发仍从 00:00 累计，次日 09:00 的自动日报仍统计前一完整自然日。非超管不能触发。以后增加收件人时，只修改独立收件人配置并重启 `qq-bot`，不需要也不应该把收件人加入超管列表：
