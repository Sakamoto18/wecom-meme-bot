...
LONGTU_QQ_ENGAGEMENT_MAX_REPLIES=4
...
真人明确 `@` 或引用机器人并收到回复后，会按群号开启默认 100 秒的群级连续话题窗口。原发起者和其他真人都可以承接同一话题，但不会逐条必回：原发起者的明确相关追问优先回复，其他相关 `may` 发言同样需要通过上下文语义价值复核，再经过默认 18 秒群级节流和 0.60 概率；每段窗口最多主动补充 4 次。只有实际的相关话题回复才会重新计算 100 秒，无关发言、附和与转向他人既不回复也不续期。窗口内再次明确 `@` 会续期，但不会更换原发起者或重置主动补充次数；群内连续 `@` 默认采用 5 秒短节流，防止多人同时点名造成瞬间刷屏。发起者明确说“别再回复了”“不许回复了”“停止回复”“结束这个话题”“到此为止”等时由程序立即关闭整段话题，其他参与者提出同类要求时只退出其本人，结束指令不受连续 `@` 节流影响。登记在 `LONGTU_QQ_PEER_BOT_USERS` 中的 Bot 永远不能加入或续期真人话题窗口。`LONGTU_QQ_ADMIN_USERS` 中的超级管理员拥有群级最高优先级：无论机器人正在和谁聊，超管发出明确自然语言结束指令或单独发送 `/stop`，都会在任何模型调用前关闭该群话题窗口、暂停该群普通主动回复 100 秒，并把所有已登记 peer Bot 直接压到硬上限，在 5 分钟循环窗口到期前不再续聊；同群已经开始生成但尚未发出的旧回复也会被抢占丢弃。`/stop` 只对超管生效，普通成员发送时静默拒绝。新的真人明确 `@` 仍可开启一段全新对话。
...
读空气的初次判定（`active-reply-decision`）和发言价值复核（`active-value-gate`）使用相同的请求前缀：不可信资料声明、完整群聊上下文，之后才附加各自原有的系统判定规则和任务指令。这样复核可以尝试复用初次判定的上下文缓存；不增加摘要调用，不丢弃历史，不复用旧判定答案。最终聊天回复的人格提示、消息顺序、生成参数和人格复核保持不变。收益取决于供应商缓存写入时机与保留策略，需要分别比较这两个调用来源的缓存输入 Token / 输入 Token；本地前缀一致性测试不能代表线上缓存已经命中。
...
QQ_USAGE_PEAK_LARGE_GROUP_ENGAGEMENT_DECISION_COOLDOWN_SECONDS=30
