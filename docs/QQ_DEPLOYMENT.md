...
媒体链路有两类独立限制：下载/解析过程最多运行 8 分钟；Provider 或 B 站播放接口已经返回视频时长时，超过 8 分钟的视频会在注册流式中转前直接拒绝，不会让 NapCat 开始下载长视频。视频大小上限仍为 500 MiB。已拿到的封面、作者和标题会先生成独立卡片，再发送“加载时长超过 8 分钟”或“提取超时”提示；封面失败不影响提示发送。
