...
- 纯 `@机器人` 没有附加文字时使用完整角色知识，关闭 thinking，不联网，直接回一句针对空喊者的角色回怼并附龙图。真人直接艾特且附带短歧义文本时，先结合上下文判断真实意图；明确问题、图片、引用、转发、视频、链接和敏感求助跳过这项检查。确认无实质请求的逗弄在同一次调用中生成一句回怼，不再搜索或追问，也不开启或刷新续聊窗口。简短追问、真实任务、判断不确定或检查失败仍交回正常链路；这类歧义消息可能多一次短判定，明确问题不增加判定调用。peer Bot 的回复频率与防循环阀门保持原逻辑。
- 角色基座统一为“脾气冲、嘴损但懂行”的群友：普通答案也保留利落判断和针对事情的讥讽，不能只加“离谱、抽象”充当人格。无实质请求艾特、直接挑衅或明确指定攻击目标时才对人开火；引用作者、无关群友与真实求助不因语气增强而被迁怒。
...
“已保存”不等于对所有人都生效：默认 `user` 范围只在 Bot 回复该用户时加载，包括该用户的私聊和群聊。全局人格需要管理员明确指定全局范围并确认保存；个人范围不会自动扩大，未确认的草稿不会进入回复。

验收时先发 `/persona list` 查看当前已保存内容，再问“你口癖呢”，Bot 应原样展示至少一条；攻击性原句会以引号示范，不向测试者开骂。日常对话按语义选择合适原句，避免近期重复和无关硬插，不保证每轮出现。普通追问直接补充答案，不再自动添加“说白了”，也不以“某某问的是”转述提问。

### 龙图显示与收藏

龙图库选中的图片按 QQ 表情类型发送（OneBot `image.sub_type=1`），关键词、tag、图片匹配、随机选图和去重仍使用原有本地图库。Bridge 在文字发送后通过 OneBot 直接发送独立龙图，并记录 `message_id` 回执；自定义表情组件不能交给 AstrBot RespondStage，否则独立图片会被误判为空而跳过。视频分享卡、图文预览仍是普通图片。具体显示尺寸由 QQ 客户端决定，原始图片不会被缩小或覆盖。

联网仍在后台核实事实，默认回复不展示媒体名括号、来源尾注或参考链接；明确问“来源呢”“原文发我”时才展示。输出校验只清理来源装饰，不删除日期、数字、条件等正文信息。

表情样式发送不要求先加入账号「我的收藏」。NapCat 的 `add_custom_face` 可以另外同步收藏，`fetch_custom_face_detail` 可以查询现有收藏；收藏是否成功需要读回核对，且受 QQ 账号容量限制。QQ 收藏不会保存本项目的 tag 和使用记录，不能用它替代本地图库数据库。
...
读空气的初次判定（`active-reply-decision`）和发言价值复核（`active-value-gate`）先发送稳定的中立规则，再发送动态群聊上下文、引用和当前任务；普通回复也先发送稳定的龙图知识/角色规则，再发送历史、搜索摘要和本轮问题。这样供应商可以复用跨消息的稳定前缀，不会复用其他群的聊天内容，也不复用旧判定答案。普通回答保留短、嘴欠、傲气和鲜明阴阳，用当前话题的具体反差直接表达判断，不再要求先给正经结论、再拼通用口头钩子；重写时也保留贴合话题的反讽。锋芒默认落在事情上，敏感求助、纯确认和单个事实不硬塞段子。语气调整不新增模型调用，不扩大上下文或主动回复频率。收益取决于供应商缓存写入时机与保留策略，需要比较改造前后的缓存输入 Token / 输入 Token；本地前缀一致性测试不能代表线上缓存已经命中。
...
会话记忆默认最多保留 50 个会话（群聊和私聊合计），通过服务器 `.env.qq` 中的 `CONVERSATION_MEMORY_CONVERSATIONS=50` 配置。超过上限时淘汰最久未更新会话的原文和摘要，独立保存的成员画像和人格训练数据不受影响。这是会话记忆容量，不是 QQ 加群或接收消息的数量限制，也不会预先分配 50 份资源；实际会话不足 50 个时，从 300 下调不会直接降低当前模型调用量或明显释放内存。已有部署需要修改真实 `.env.qq` 并重建容器，仅更新模板不会覆盖线上配置。
