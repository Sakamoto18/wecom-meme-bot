...
- `source-display.js` 按当前问题决定是否展示来源；默认隐藏引用尾注，明确询问出处时保留。搜索证据仍进入后台核实，事实、数字和不确定性不因此删减。
...
- 纯艾特或语义确认的无实质请求艾特：只回一句针对发言者的角色回怼并附龙图，不联网，不开启或刷新续聊窗口。`mention-intent.js` 仅检查真人直接艾特中的短歧义文本，按上下文语义决定，不以短句或词表直接判攻击；同次调用完成判断和回怼。真实问题与材料走原链路，歧义检查失败保守回到正常回答；peer Bot 不接入这项检查。
...
普通回答中的图片不会自动写入图库。明确攻击、纯艾特、无实质请求艾特回怼和管理员绑定图可强制发图；普通场景需要场景语义命中，避免每句话随机附图。

Bridge 的 `_deliver_reply_chains` 让文字走 AstrBot 原发送流程，独立龙图直接调用 OneBot 并验证 `message_id`，保留 `sub_type=1`。原因是 AstrBot RespondStage 按精确组件类检查空消息，自定义表情单独成条会被跳过；不能仅测试最终序列化。`test/verify_astrbot_sticker_pipeline.py` 在实际 AstrBot 环境用 recording client 验证群聊/私聊、分段开/关、文字后附图和纯图片，不向 QQ 发消息。
