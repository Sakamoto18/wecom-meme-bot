...
QQ 和企微都支持管理员通过聊天添加、强制添加、删除和撤销龙图，也支持把文字别名精确绑定到某张图片。仓库内 240 张龙图已通过 macOS Vision 本地 OCR 生成唯一文字别名，运行时不依赖大模型多模态；发送“发 + 别名”会直接按本地资源映射发图。管理员可附图或引用图片发送“以后发赛尔号就用这张图”，也可先说“把这张图加进图库”，再在 15 分钟内说“图片标记赛尔号”。手动绑定会覆盖 OCR 结果；普通对话明确提到手动别名时仍正常生成文字，但末尾附图会改用对应绑定图，例如“辱骂一下赛尔号”。视觉复核、OCR、查重和图片定位都在本地完成，动态素材、软删除与手动绑定保存在 `data/longtu-library/`。QQ 的权限、角色观察和身份钢印配置见 [QQ 部署文档](docs/QQ_DEPLOYMENT.md)。
