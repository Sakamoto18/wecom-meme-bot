...
QQ 和企微都支持管理员通过聊天添加、强制添加、删除和撤销龙图，也支持把文字别名精确绑定到某张图片。仓库内 240 张龙图已通过 macOS Vision 本地 OCR 生成唯一文字别名，运行时不依赖大模型多模态；发送“发 + 别名”会直接按本地资源映射发图。管理员还可附图或引用图片发送“以后发赛尔号就用这张图”，手动绑定会覆盖 OCR 结果。视觉复核、OCR、查重和图片定位都在本地完成，动态素材、软删除与手动绑定保存在 `data/longtu-library/`。QQ 的权限、角色观察和身份钢印配置见 [QQ 部署文档](docs/QQ_DEPLOYMENT.md)。

QQ 群中只有明确 `@机器人` 才会触发回复；允许群的普通消息只用于静默更新成员身份和群聊语境，不会主动插话。成员本人发言后才会确认其昵称别名；别人 `@` 某人只登记稳定 QQ 身份，不会替对方确认昵称。已确认且在群内唯一的历史昵称可以在纯文字中锁定第三方目标，无需再次 `@`；重名或未确认昵称不会强行猜测。内部匿名编号只用于后台身份消歧，所有出站回复都会硬过滤，绝不发到群里。所有以 `/` 开头的 AstrBot 或其他插件指令（包括 `/w`、`/help`）均被龙图 Bridge 完全忽略，不回复、不拦截，也不写入本 Bot 的群聊记忆。
