...
QQ 和企微都支持管理员通过聊天添加、强制添加、删除和撤销龙图。视觉复核完全在本地完成，不要求大模型具备多模态能力；动态素材与软删除记录保存在 `data/longtu-library/`。QQ 的权限、角色观察和身份钢印配置见 [QQ 部署文档](docs/QQ_DEPLOYMENT.md)。

QQ 群中只有明确 `@机器人` 才会触发回复；允许群的普通消息只用于静默更新成员身份和群聊语境，不会主动插话。所有以 `/` 开头的 AstrBot 或其他插件指令（包括 `/w`、`/help`）均被龙图 Bridge 完全忽略，不回复、不拦截，也不写入本 Bot 的群聊记忆。
