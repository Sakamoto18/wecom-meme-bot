...
普通对话启用大模型后，机器人会先完成流式文本回复，再通过同一条长连接主动推送一张本地 JPG/PNG 龙图。正常语聊会将用户原话和模型文案与图库文字做本地场景匹配；OCR 保存的是图片文字/场景标签，不会要求用户记住整句，也不会把整句当成手动口令。例如用户输入“玩原神玩的”，会匹配 OCR 文字中含“原神”的龙图。只有明确说“发张龙图”时才保持纯随机；未命中场景标签时再使用会话洗牌策略。管理员手动别名优先级最高。之所以拆成两条消息，是因为部分企微客户端不会渲染流式回复中的 `msg_item` 图片。附图准备或主动推送失败时，已经发送的模型文本不受影响。

龙图选择采用按会话持久化的洗牌袋：同一会话抽完整个可用池之前不重复，并在最近 12 次内尽量避开感知特征相近的场景。选择进度保存在 SQLite，进程重启后不会清空。

QQ 和企微都支持管理员通过聊天添加、强制添加、删除和撤销龙图，也支持把手动别名精确绑定到某张图片。仓库内 240 张龙图已通过 macOS Vision 本地 OCR 生成文字场景标签，运行时不依赖大模型多模态；只有管理员手动绑定的别名才是“发 + 别名”的精确口令。管理员可附图或引用图片发送“以后发赛尔号就用这张图”、 “这个是耄耋”，也可先说“把这张图加进图库”，再在 15 分钟内说“图片标记赛尔号”，或直接说“强制添加并且标记为赛尔号”。使用“别名列表”或“标记列表”可检查手动别名和 OCR 场景标签示例。如果自动复核失败，程序会提示引用图片后发送“强制添加这张龙图”；如果图片已存在，则会明确提示已在图库中。手动绑定会覆盖同名 OCR 标签；普通对话会结合用户原话和模型文案匹配关键词，命中后仍正常生成文字但末尾附对应图片，例如“辱骂一下赛尔号”或“玩原神玩的”。视觉复核、OCR、查重和图片定位都在本地完成，动态素材、软删除与手动绑定保存在 `data/longtu-library/`。QQ 的权限、角色观察和身份钢印配置见 [QQ 部署文档](docs/QQ_DEPLOYMENT.md)。
